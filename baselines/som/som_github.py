...
class SOM:
    ...
    """
    def __init__(self, input_shape, map_size_n, num_expected_iterations):
        print (input_shape)
    ...
    def train(self, session, input_x): #TODO: try training a batch all together, to optimize gpu usage?
        # Compute the winning unit
        bmu_index, diff = session.run([self.bmu_index, self.diff], {self.input_placeholder:[input_x], self.current_iteration:self.num_iterations})
    ...
    def get_batch_winner(self, session, batch_input):
        """
        Returns the index of the units in the network that best match each batch_input vector.
        """
        indices = session.run([self.bmu_index], {self.input_placeholder:batch_input, self.current_iteration:self.num_iterations})
